...
Welcome to the Sales Development Organization at GitLab! We are a team designed to drive [results for customers](/handbook/values/#results) through both inbound and outbound strategies. Our structure is designed to maximize efficiency, responsiveness, and creativity in our Groove efforts.
...
| [Sequence Creation Board](https://gitlab.com/gitlab-com/marketing/sales-development/-/boards/5540104) | The board we manage all new, and in-flight, flows and messaging campaigns. |
...
| [Groove flow naming convention](https://docs.google.com/spreadsheets/d/12YXwIE126g0rmmNi7QYy2s0SewIV-2JWCr9YUPcqfaQ/edit#gid=0) | Document outlining the naming convention for Groove sequences. |
...
| View    | Description                                                                                                                                                                                                       |
|---------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| B1 View | Includes MQL's, Hot contacts that require quick follow up. NOTE: Not all of these contacts will be in your Actively Working Accounts list as all contacts where you are BDR Assigned on account are in your ownership, so please ensure that you set the account BDR Prospecting Status to Actively Working if you engage in discussions with the contact so that you can continue working the account and expand your Groove, otherwise any new lead MQLs for the account will route to the SDRs .                                                                                                                                                        |
| B2 View | Contacts related to Actively working accounts that you can choose to flow                                                                                                                      |
...
**1. Determining High-touch or Low-touch flow usage:**
...
1. Research the appropriate Groove collections for relevant collateral to be used. Most typically our [High-Touch](https://web.Groove.io/sequences?queryFilters=%5B%7B%22attribute%22%3A%22collection%22%2C%22operator%22%3A%22is%22%2C%22value%22%3A%5B%2269%22%5D%7D%5D&sortBy=recent&sortDirection=desc) or [Low-Touch](https://web.Groove.io/sequences?queryFilters=%5B%7B%22attribute%22%3A%22collection%22%2C%22operator%22%3A%22is%22%2C%22value%22%3A%5B%2271%22%5D%7D%5D&sortBy=recent&sortDirection=desc) collections.
...
#### How to enroll to Groove

[Groove.io](https://www.Groove.io/) is a tool used to automate emails in the form of sequences. Users can track open rates, click-through rates, response rates for various templates, and update flows based on these metrics. Groove.io also helps to track sales activities such as calls. All emails/calls/tasks that are made through Groove.io are automatically logged in Salesforce with a corresponding disposition. See below for a list of current call dispositions, what they mean, and scenarios on when to use each of them.

1. Go to your SDR views on SFDC and select the lead you want to sequence.
1. Press `Import to Groove`.
1. Work from the Groove extension on Chrome to then select the flow you want to enroll the lead in.
1. If manual steps are required for the flow that you chose, select `Edit Steps`. Go through each step you want to edit, paying especial attention to manual variables.
1. Once the first step of an Groove flow is complete, the lead status will automatically change from `MQL, Inquiry, or Raw` to `Accepted` status, marking that you are actively working on this lead.
...
1. When looking at your qualifying view, flow leads that have no recent last activity + no active tasks + are not actively being sequenced into one of our follow up flows that have the `Follow up Ruleset Sequences`.
1. Tasks and your pipeline can be then managed via the Groove [360 view.](https://support.Groove.io/hc/en-us/articles/214806328-Navigating-the-360-View-Dashboard)
...
1. If a lead finishes an Groove flow without responding, the lead status will automatically change to `Unresponsive` or `Recycle` in seven days if there is still no response.
1. If a lead responds, schedule a call/meeting using Groove's meetings feature.
1. Manually change lead status if you don't use an Groove flow to reach out to someone or if you need to unqualify a lead for bad data etc.
1. If you check the Inactive lead or Inactive contact checkbox, signifying that this person no longer works at the company, any running flow will automatically be marked as finished.
1. If you notice your leads are being reassigned to Inquiry Queue, that's due to a scheduled clean up job in Traction that updated `Status = Inquiry` to Inquiry Queue. To resolve this, add your leads to an Groove sequence. Note: The scheduled clean up runs daily at 10:30 PM EST/EDT.

#### Groove Collections

Ways to organize similar flows and snippets

- **Common Collections**
  - [FY24 Global Inbound High Touch Collection](https://app1a.Groove.io/sequences?queryFilters=%5B%7B%22attribute%22%3A%22collection%22%2C%22operator%22%3A%22is%22%2C%22value%22%3A%5B%2269%22%5D%7D%5D&sortBy=recent&sortDirection=desc)
  - [FY24 Global Inbound Low Touch Collection](https://app1a.Groove.io/sequences?queryFilters=%5B%7B%22attribute%22%3A%22collection%22%2C%22operator%22%3A%22is%22%2C%22value%22%3A%5B%2271%22%5D%7D%5D&sortBy=recent&sortDirection=desc)
  - [FY24 Inbound Languages Collection](https://app1a.Groove.io/sequences?queryFilters=%5B%7B%22attribute%22%3A%22collection%22%2C%22operator%22%3A%22is%22%2C%22value%22%3A%5B%2270%22%5D%7D%5D&sortBy=recent&sortDirection=desc)
  - [Events Collection](https://app1a.Groove.io/sequences?queryFilters=%5B%7B%22attribute%22%3A%22collection%22%2C%22operator%22%3A%22is%22%2C%22value%22%3A%5B%2266%22%5D%7D%5D&sortBy=recent&sortDirection=desc)
  - [Good Outbound Collection](https://web.Groove.io/sequences?queryFilters=%5B%7B%22attribute%22%3A%22collection%22%2C%22operator%22%3A%22is%22%2C%22value%22%3A%5B%2276%22%5D%7D%5D&sortBy=recent&sortDirection=desc)
  - [Testing Outbound Collection](https://web.Groove.io/sequences?queryFilters=%5B%7B%22attribute%22%3A%22collection%22%2C%22operator%22%3A%22is%22%2C%22value%22%3A%5B%2275%22%5D%7D%5D&sortBy=recent&sortDirection=desc)

#### Groove Tags

A method of distinguishing flows and snippets from others. Use tags to help narrow down which flows you should be using in a particular situation

##### Common Tag Examples

- **Primary-** a flow created by SDR leadership that should be used by all reps to follow up with inbound leads
- **High Touch/Low Touch-** Distinguish if the flow is high or low touch
- **Region-** Can be used to show which region the sequence
- **GTM-** The message trying to be delivered. Ex. Security, CI/CD ect.
- **Inbound/Outbound-** Whether the flow is meant for inbound or outbound prospecting
- **Language-** What language the flow is written in
...
- **Common Snippet Examples**
  ...
  - [Objection Snippets](https://app1a.Groove.io/snippets?direction=asc&order=owner&tags%5B%5D=objection)
  - [Support Snippets](https://app1a.Groove.io/snippets?direction=asc&order=owner&tags%5B%5D=Support)

#### New Groove Sequence/Workflow creation

In an effort to better iterate, and refresh, our Groove content and workflows, we have a structured process for BDRs to create/measure new Groove sequences.
...
#### Follow Up Groove Sequences

- For SDRS and BDRS that want to flow leads in their qualifying views we created [this video](https://www.youtube.com/watch?v=IKE24LRJbcc) to help you create a workflow around this follow up [sequence.](https://app1a.Groove.io/sequences/9383)

- The same video also showcases the [Auto Follow Up](https://app1a.Groove.io/sequences/9543) to Book a Meeting flow SDRs and BDRs can use when a prospect responds to one of their sequences.

- For [guiding prospects](https://gitlab.com/gitlab-com/marketing/sales-development/-/issues/817) to web-direct, we also have [a flow that you should use](https://web.Groove.io/sequences/14652/overview) to ensure that you streamline their buyer's journey.

#### White Glove Event Follow Up flow (SAEs Included)

For more personal events, such as Executive Roundtables, there many be more detailed Last Event Notes that have an ask to include the people who interacted with the prospect at the event. For these situations we have a [specialized white glove sequence.](https://app1a.Groove.io/sequences/12883)

 The goal of this flow is to reach out to the prospect quickly and to include our SAEs/AEs in the conversation.
...
#### Other Groove Training Resources
...
- GitLab Edcast: [Advanced Groove Training](https://gitlab.edcast.com/journey/weeks)
- Best Practices: [Our Groove Handbook Page](/handbook/marketing/marketing-operations/Groove/)
...
BDRs will work closely with their dedicated SAE or AE to choose which accounts move into "Actively Working". Together they will build a strategy for those companies. It is crucial that Groove is very intentional and strategic. When reaching out, we want BDRs to offer value and become a trusted advisor, ensuring a positive impression is left whether there is current demand or not.
...
This comprehensive research will provide valuable insights for crafting effective Groove strategies further down the funnel. Regarding account research, in addition to our B2B database toolstack, we also use 6Sense to pre-qualify the accounts that should be considered for our pipelines since 6Sense scores accounts based on their market and profile fit.
...
Populate the BDR Next Step Date to be at the end of the account's flow length (typically one month out)
...
1. **Schedule IQM**: For booking meetings, use [Groove Meetings](https://www.Groove.io/resources/blog/meetings-pr) to expedite the experience for prospects.
...
1. **Rebooking no-shows**: The SDR's responsibility lies only in rebooking no-shows using the [Groove rebook sequences](https://web.Groove.io/sequences?search=no%20show&sortBy=recent&sortDirection=desc).
   - Conduct Groove for 2 weeks. If IQM cannot be rescheduled, AE will unqualify the opportunity.
...
| If activity shows up on the record, and the activity is a connected call with notes, verify that the activity matches in Groove. | Activities on SFDC can be edited freely and therefore questioned by our audit team. | Explain that Groove is a SSoT that can be used as irrefutable proof of Groove. Even if other systems fail, Groove is always used as the last back-up. |
...
1. flow all leads if you're reaching out
...
From FY24Q4 onwards, we will pilot [automatic triggers](https://support.Groove.io/hc/en-us/articles/221361788-How-To-Create-an-Groove-Trigger) through Groove.

#### Automatic High PTP Sequence

As a first iteration, we will be automatically enrolling leads that indicate a high Propensity to Purchase (PTP) score to a full High Touch Groove sequence. The filters applicable for this trigger are:
...
We have an additional process where leads that [were automatically moved to disqualify lead status because of a wrong email]https://gitlab.com/gitlab-com/marketing/marketing-operations/-/issues/7718(), BUT have a phone number associated to them, will automatically be enrolled to a [sequence.](https://web.Groove.io/sequences/13842)
...
| Step                                           | Description                                                                                                                                                            |
|------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Gather billing and end-user details from the reseller | - Billing company name/address: <br> - Billing company contact/email address: <br> - End-user company name/address: <br> - End-user contact/email address: <br> - Snippet in Groove |
...
| Priority 1 | Priority #, Any Details | Future based next steps | Hyper-personalized, simultaneous Groove, creative, direct mail, ads, groundswell, events |
| Priority 2 | Priority #, Weekly Plan or Length of flow | Future based next steps depending on account strategy | Include the weeks Groove and overall strategy |
| Priority 3 | Priority # | Update Next Step Date to next months Groove | BDR Next Step, What kind of nurture (Product Updates, Events, Ect) |
...
| Hyper-personalization    | The concept of combining real-time data extracted from multiple sources to create Groove that resonates with prospects on an individual level.                             |
...
| Vartopia Partner Account | If populated, the lead/contact is worked by a partner, and Sales Dev should not proceed with Groove.                                                                     |
| Actively Working Status   | Accounts chosen for strategic Groove based on alignment, news, initiatives, etc. Recycles after 10 weeks if not worked.                                                    |
...
- These saved searches will pull the relevant decision-makers per segment, and through ZoomInfo workflows do the following:
  ...
  - Automatically enroll SMB leads to the fully automated flow [here](https://web.Groove.io/sequences/13896/overview)
...
| 1:1 Accounts Dashboard | 2. Research Accounts to be Recycled | **For accounts in AW:** <br> These reports are nearing their automated recycle date, which by default is two months after their actively started work date. This report acts as a fallback report to the report above. A BDR should have already reviewed these accounts as part of their actively working account screen done in the previous step. This report will act as a second reminder. | In theory an outbound account will have received at least 30 days worth of Groove since most flows we use have at least a 30 day cadence. It's fine to leave an account in a team member's pipeline until it's removed automatically in hopes of getting some inbound MQLs assigned to the BDR. Regardless, there's a business decision to be made about whether expecting inbound MQLs is a sound strategy, based on the team members quota attainment and overall pipeline at the moment, and whether replacing the about to be expired for another account is a better route. |
...
| 1:1 Account Dashboard | 16. Review individuals in flight. | **For accounts not in AWA:** <br> This report shows a breakdown of prospects currently on a sequence. Should be reviewed for any further actions to be taken. | Consider whether adding more individuals to a flow for a particular account might make sense, or whether the individuals from the top prospect accounts should be getting additional Groove outside of the flow steps. |
...
- **Example Sales_Dev_fyi channel post titles**
  ...
  - `[All BDRs] | [Operations - Groove Process Cleanup] | [🧠 Need to Know ]`

  - `[EMEA Enterprise Land] | [Operations - New Groove Event Sequence] | [🚨 Action Required ]`
...
- If a regionalized flow can be translated to English, then the English copy of it will be added to the GC while the local variant will be added to the Team Collection.
- Field Marketing event flows and any other similar type will not be considered for the Good Collection.

**2. flow needs to have completed the requirements set out on the issue descrption**

- At least 200 people enrolled and/or having completed the flow on a timely manner.
- ( flow has over 5% reply rate AND No less than 50% negative sentiment rate. ) OR flow has over 5% meeting set rate.

#### Things to look out for when reviewing a flow as a Manager

- Does the messaging of the flow follow Command Of the Message?
  - Are all emails formatted appropriately with pain-probing questions and value propositions?
  - Are the relevant value drivers for the personas targeted by this flow used?
...
- Is the purpose of the flow clearly articulated on the issue description?
...
**A:** Our MOPs team has set up a process to address any data subject requests for deletion or access to data. If you receive a request for deletion or access to data, please re-direct inquiries to the [Personal Data Subject Request.](https://support.gitlab.io/account-deletion/) This messaging snippet can be [used and tailored.](https://web.Groove.io/snippets/1903)

**Q:** Why are BDRs no longer the Account Owner in Groove? <br>
**A:** BDRs are no longer listed as the account Owner in Groove to improve Sales Dev and Sales visibility across all prospects/accounts (excluding PubSec). If you want to filter to see just your accounts, please use the 'BDR Assigned' field.
...
**A:** BDR's can work leads in all statuses if they are related to their "Actively working" accounts. Groove to these leads would be part of their account strategy.
...
**A:** Marketing Operations has a daily scheduled clean at 10:30 PM EST/EDT that update Status = Inquiry to Inquiry Queue. To prevent the reassignment, you'll want to update the leads from Inquiry to Accepted or add to an Groove sequence.
...
**A:** Partner leads will not be asigned to BDRs or SDRs as they will be assigned to the Partner Queue. To tell whether a lead is associated to a Partner Account, check their `Vartopia Partner Account` - if it's empty, you are may proceed with Groove.
...
|                                    | - Work lead records within Salesforce by leveraging relevant flow libraries in Groove.                                                                                            |
...
|                                    | - flow steps to be actioned within the same day of them being due. Move steps to a further date only because of national holidays affecting Groove (i.e., Christmas day).      |
...
|                                           | - Collaborate with the Field Marketing team in terms of event Groove.                                                                                                                |
...
| Low vs High Touch flow Usage    | - Over 70% of inbound leads enrolled to High Touch sequences.        |
...
- Lunchtimes are good for Groove as most prospects have meeting blocks for three hours in the morning and then two hours mid-afternoon.
