...
In order to ensure that all [SAFE](/handbook/legal/safe-framework/) data is being stored in appropriate schemas all models that are downstream of [source models with MNPI data](/handbook/marketing/data-team//how-we-work/new-data-source/#mnpi-data) must either have an exception tag or be in a restricted schema in `PROD`. This CI Job checks for compliance with this state.
...
- If no optional arguments are passed into the CI job, it will run with the default arguments described in [Automating roles.yml: Common Templates](/handbook/marketing/data-team//platform/#common-custom-templates) section of the handbook.
...
1. `DATABASES_TEMPLATE`:
    - Defaults to None, but accepts any JSON string, see this ['Databases' handbook section](/handbook/marketing/data-team//platform/#databases) for more details/examples.
1. `ROLES_TEMPLATE`:
    - Defaults to 'SNOWFLAKE_ANALYST' role and 'DEV_XS' warehouse, but accepts any JSON string, see this ['Roles' handbook section](/handbook/marketing/data-team//platform/#roles) for more details/examples.
1. `USERS_TEMPLATE`:
    - Defaults to the standard user entry, see ['Users' handbook section](/handbook/marketing/data-team//platform/#users) for more details/examples. This value can be overriden with any JSON string, but should not be necessary.
