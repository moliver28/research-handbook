...
- should cc (mention) other team members who are subject matter experts (SME) or [functional DRIs](/handbook/marketing/data-team//#-team-organization) for their feedback
...
[Read](/handbook/marketing/data-team//how-we-work/triage/) the FAQ and common issues.
...
Depending on the nature and impact of the [incident](/handbook/marketing/data-team//how-we-work/#incidents) a severity needs to be determined. Currently we don't have a decision matrix in place. To determine the severity, take the following aspects into consideration:

- In case of a data source or data pipeline incident, check the [Data Source Tier](/handbook/marketing/data-team//platform/#data-sources).
...
In a situation when [Service ping](/handbook/marketing/data-team//data-catalog/saas-service-ping-automation/#service-ping-overview) fail while it generates metrics, we should be informed either via `Trusted data dashboard` or `Airflow` log - generally, the error log is stored in `RAW.SAAS_USAGE_PING.INSTANCE_SQL_ERRORS` table. Follow the instructions from the link [error-handling-for-sql-based-service-ping](/handbook/marketing/data-team//data-catalog/saas-service-ping-automation/#error-handling-for-sql-based-service-ping) in order to fix the issue.
...
Full refresh required as per instructions from [dbt models full refresh](/handbook/marketing/data-team//platform/infrastructure/#dbt-models-full-refresh).
...
If the pipeline is broken it needs to be fixed, currently we are working on defining SLO's for our data assets. For our data extraction pipelines, there is a comprehensive overview [here](/handbook/marketing/data-team//platform/#extract-and-load/).
