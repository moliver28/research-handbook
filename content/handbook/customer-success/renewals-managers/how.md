...
The renewals team organizes it's coverage of a customer subscriptions around particular key activities during a customers journey through the renewal process. During each stage we prescribe specific engagements that encourage renewals managers to deliver distinct, stage relevant customer outcomes.

| Time Frame | Renewals Activity | Customer Outcomes (Qualitative)                                          |
|--------------|-------|--------------------------------------------------------------------|
| Mo. 1  | 1. Activate      | Customer welcomed/back, environment set-up/affirmed, startup issues identified/triaged, licenses utilized |
| Mo. 3 | 2. Adopt | Customers encouraged to optimize platform, early risk signals identified/mitigated, subscriptions show high utilization |
| Mo. 6    | 3. Advocate | Customers identifying expansion opportunities, verbalizing advocacy        |                     |
| Mo. 9| 4. Prepare     | Comfortable and starting the commercial portion of the renewal lifecycle     |
...
- **Opportunity Coverage**
  - Renewals managers primary focus is managing renewal opportunity `ARR basis` ; coverage for new business and add-on's `growth NetARR` should default to the account executives team
...
- Churn exceptions are possible, but not guaranteed. Each circumstance will be considered independently. Some common scenarios considered for churn exceptions are:
  1. Misrepresented or temporary Churn/Contraction
  - a. Near-term add-on contraction "Insta-churn" (true up negotiations, budget optimizations)
  - b. Subscription migration and consolidations - where we can prove that users simply moved to another subscription (not-merged) on another opportunity, or to another account, "User Migration"
  ...
  - a. Large/Influential deals where a single churn event has an outsized impact on attainment


- Timing: Have all exceptions submitted by the end of quarter.
...
- Submission
  - When submitting a churn exception, chatter wth this syntax [Churn Exception Request] Details and context (link to supporting doc) CC: Manager for approval, CC: Director for approval. Manager/director will add sales-comp for final consideration if applicable. 
  - For "User Migration" exceptions, no chatter is necessary, just follow the user migration process (which is to use #usermigration in the renewal manager notes, with the link to the corresponding migration opp and associated netARR)
