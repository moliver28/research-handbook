...
User can also use one of predefined Chat commands to perform
[refactoring, explanation or writing tests](https://docs.gitlab.com/ee/user/gitlab_duo_chat.html#explain-code-in-the-ide) for a selected code.
...
sequenceDiagram
    ...
    Note over IDE,LLM: GraphQL API
    IDE->>GLR: opens Duo Chat and sends `/refactor` messages with selected code
    GLR->>IDE: accepts the request and enques it
    Note over IDE,LLM: Asynchronous job
    GLR->>AIGW: builds prompt and sends AI request
    AIGW->>LLM: sends AI request
    LLM->>AIGW: receives response
    AIGW->>GLR: receives response
    GLR->>IDE: sends websocket message with response
```
