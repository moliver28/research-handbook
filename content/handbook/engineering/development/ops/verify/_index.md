...
We track our technical debt using the following [Verify Technical Debt issue board](TBD), where we track issues in the planning phase.
...
1. Issue scheduling: The Engineering Managers will review the [Verify Technical Debt issue board](TBD), *filtered on their team*, and recommend issues to schedule for upcoming milestones by adding them to the [planning issue](#planning) for a milestone or communicating with Product a need to schedule the issue in an upcoming milestone. The team strives to have 20% of a sprint's capacity filled with Tech Debt issues.
