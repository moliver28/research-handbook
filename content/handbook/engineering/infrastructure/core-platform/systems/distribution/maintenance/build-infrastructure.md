...
Many of these build processes require higher computing power and resources, and
hence Distribution maintains own infrastructure to provide these resources.

### Runner manager machines

Distribution team maintains 2 runner manager machines - one to be used in
GitLab.com and the other to be used in dev.gitlab.org. The configurations of
runner managers are maintained in the
[chef-repo](https://gitlab.com/search?search=f%3Adistribution&project_id=26735010&group_id=1112072)
project, and the secrets to be used together with these configurations are
maintained in [Vault](https://vault.gitlab.net).

The runners are named in the syntax `runners-manager-distribution-(com|dev)-(blue|green)-<count>`.
...
discusses about using a blue/green deployment for easier upgrades of GitLab
Runner in these runner manager machines. So, currently, only a single "`blue`"
runner manager is available for each of GitLab.com and `dev.gitlab.org`.

### Build machines

Both `com` and `dev` runners spins up identical machines using `docker-machine`
