...
1. If you managed to mitigate the problem and make the pipeline pass, but the issue could comeback in the future, then:
   ...
   - For each environment that you has to restart with `@gitlab-bot retry_job <job-id of the review-* job>`, now manually stop the environment with `@gitlab-bot retry_job <job-id of the stop-review-* job>`. This is necessary because our `auto-stop-in` does not work when we manually retry the job that re-creates the environment. See chart issue [#5326](https://gitlab.com/gitlab-org/charts/gitlab/-/issues/5326).
