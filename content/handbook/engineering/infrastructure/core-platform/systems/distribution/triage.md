...
1. If you managed to mitigate the problem and make the pipeline pass, but the issue could comeback in the future, then:
   ...
   - For each environment that you've to restarted with `@gitlab-bot retry_job <job-id of review-*>`, now manually stop the environment with `@gitlab-bot retry_job <job-id of stop-review-*>`. This is necessary because our `auto-stop-in` does not work when we manually retry the job that re-creates the environment. See chart issue [#5326](https://gitlab.com/gitlab-org/charts/gitlab/-/issues/5326).
