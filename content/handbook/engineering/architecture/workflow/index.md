...
1. Work with the stakeholders and [domain experts](#domain-experts) to get your
   design reviewed.  Design docs don't need to be comprehesive nor complete on
   the first iteration. They can be refined and enriched with details in
   iterations, as we discover important design aspects, fundamental design areas,
   key decisions.
1. Start implementation as soon as possible!
1. Iterate on the design doc in parallel as you discover new details, make
   fundamnetal design decisions, learn more about how you want to proceed with
   the implementation and add / refine the content of the document.
...
A design doc is usually divided into multipe sections:
...
something and what is our approach to get the thing done. The audience of the
here is very broad, so it is important to avoid using a lot of technical
language.

#### Objectives

If the architecture design workflow is being used it means that what we want to
accomplish is going to be non-trivial and presumably expensive. An investment
like this should have a solid business justification, we should know what is
going to be a business impact. It is recommended to add a note about expected
business results to a design doc.
...
We recommend documenting key decisions made about fundamental aspects of the
design in the design doc. These decisions are going to become important
...
done and decisions that has beenmade. As such, a design document should be
...
architectural choices more quickly. If you are using an immutable decisions log
and short and concise overview updating the design doc should not require a lot
of effort.
