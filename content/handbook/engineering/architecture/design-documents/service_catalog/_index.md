...
This document describes how we want to evolve our metrics- and service
catalogue into a single definition in YAML for each service. So we have a
single source of truth describing a service. The YAML is inspired by a
Kubernetes kind with an associated spec that can easily be validated.

The service catalog entry should be the one-stop-shop for everyone
that wants to know about a service's ownership, deployment, monitoring
& alerting and deployment properties.

This catalog entry is also the source for generating a bunch of
related resources required for monitoring.
...
The metrics-catalog is currently defined in Jsonnet and contains everything
needed for monitoring the service and routing alerts to the relevant
groups. This includes labelling information, deployment type, and
ownership information.

The service-catalog is currently defined as a single YAML file that
lists all services with information about labelling, ownership, links
to documentation and monitoring resources.

This means a lot of information is duplicated. We need to define a
service in both places, meaning we often forget one or the other.
...
By unifying both definitions into a YAML file, we make this more
accessible to everyone at GitLab first. Different GitLab deployments
can have a different service-catalog that is used to generate
monitoring resources.

This also opens up the path to productizing our monitoring of GitLab
installations. Because we ourselves will have used different service
catalog definitions. We could look into exposing one as a default
configuration for a monitoring-mixin ([scalability#2832](https://gitlab.com/gitlab-com/gl-infra/scalability/-/issues/2832)).
...
1. Consume the generated service-definitions in new projects to
   ...
   At this moment, it's not unlikely that we need to adjust our
