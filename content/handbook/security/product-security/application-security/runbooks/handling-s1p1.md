...
Mitigation of critical security issues has to strike a balance between securing GitLab and our users as fast as possible and doing it in a reliable way.
...
- Is it automatable and easily exploitable at scale?
...
- Unpatched critical severity vuln (CVSS 9+) being exploited in the wild (if there's evidence that it's being exploited on .com or on customer self-managed installs, we should release a patch ASAP)
- Unpatched critical severity vuln (CVSS 9+) with PoC/exploitation code publicly available (it's only a matter of time before customers get pwned)
